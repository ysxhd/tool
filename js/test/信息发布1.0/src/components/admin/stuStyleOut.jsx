/*
 * @Author: JudyC 
 * @Date: 2018-01-05 10:29:25 
 * @Last Modified by: JudyC
 * @Last Modified time: 2018-03-20 17:07:21
 * 学生风采外
 */
...
export class StuStyleOut extends Component {
  constructor(){
    super();
    this.state = {
      data:[],          //教师风采数据
      ...
      <div className="cjy-so"> 
        ...
        </div>
        {
          this.state.data.map(dt => (
            <AdminStyle key={dt.uid} data={dt} showModalEdit={this.showModalEdit} showModalSet={this.showModalSet} getData={this.getData} type="1"/>
          ))
        }
        ...
        <MaskLayer ref="myMask"/>
        {
          ...
          ? <PubStoryStyle hideModalPss={this.hideModalPss} getData={this.getData} type="3"/>
          : ''
        }
        <Modal className="cjy-modal" destroyOnClose="true" title="编辑风采" footer={null} visible={this.state.editVisible} onCancel={this.hideModalEdit}>
          <EditStoryStyle hideModalEdit={this.hideModalEdit} type="3" uid={this.state.uid} getData={this.getData}/>
        </Modal>
