 ...
 * @Last Modified time: 2018-03-21 13:24:01
 */
...
export class TeaClsSpecial extends Component {
  constructor(){
    super();
    this.state={
      dataList:[],  //存储内容列表的数据
 ...
 getClsSpeci(){
  var that = this;
  let req = {
    action: 'api/web/teacher_class_brand_management/get_class_motto',
    data: {
  ...
  _x.util.request.formRequest(req, function (ret) {
    //更新父组件的数据
    that.props.changeData("clsS",ret.data);
  });
}
...
clwPsNoFun(id,num){
  var that = this;
  let req = {
    action: 'api/web/teacher_class_brand_management/audit_class_motto',
    data: {
      id: id,
      status:num
    }
