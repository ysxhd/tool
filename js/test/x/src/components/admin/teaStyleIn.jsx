/*
 * @Author: JudyC 
 * @Date: 2018-01-05 10:27:54 
 * @Last Modified by: JudyC
 * @Last Modified time: 2018-03-20 17:07:26
 * 教师风采内
 */
...
export class TeaStyleIn extends Component {
  constructor(){
    super();
    this.state = {
      data:{},              //头部信息
  ...
  render(){
    return (
      <div className="cjy-si"> 
        <StoryStyleInfo getPicData={this.getPicData} data={this.state.data} getData={this.getData} type="2"/>
          ...
          <div>
            {
              this.state.picData.map((pd,index) => (
                <div className="cjy-si-picBox" key={pd.uid} onClick={()=>this.preview(index)}>
                  <StoryStyleInPic  picData={pd} showModalEdit={this.showModalEdit} ids={this.state.ids} listOnChange={this.listOnChange} getPicData={this.getPicData} getData={this.getData}/>
                </div>
              ))
            }
          </div>
