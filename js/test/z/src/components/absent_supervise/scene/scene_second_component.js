 ...
 * @Last Modified time: 2018-05-11 15:54:13
 * 缺考 -> 现场监考组件
 */
              ...
              <Column title="机构名称" dataIndex="orgname" key="orgname" className="ljc-col-font-blue"
                render={(text, record, index) => {
                  ...
                  if (record.orgTypeId === 4) {
                    return <Link key={index} to={`/absent_supervise/site_invigilate/detail/${retId}`}>{text}</Link>
                  } else {
                    return <Link key={index} to={`/absent_supervise/site_invigilate/${retId}`}>{text}</Link>
                  }
                }}
              ></Column>
